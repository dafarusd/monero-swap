...
test('live: reads offers and a finished swap from the Sepolia contract', async () => {
  ...
  for (const o of offers) {
    ...
    assert.ok(o.asset.symbol.length > 0);
    assert.equal(e.xmrFor(10n ** 16n, o.xmrPerAsset), o.xmrPerAsset / 100n);
  }
  console.log(`open offers right now: ${offers.length}`);
